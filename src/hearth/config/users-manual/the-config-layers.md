...
| **`config/devices.toml`** | **Hearth** (pairing, and *forget*) | **Don't hand-edit.** Read it to see which devices can be talked from; pair a device to add one, and use *forget* on the launch page to take one away | [The phone lane](the-phone-lane-away-mode.md) · `docs/runbook/02.5-control-panel/paired-devices.md` |
