...
| [`the-openclaw-voice-lane.md`](the-openclaw-voice-lane.md) | ⚠️ **UNTESTED** — how OpenClaw *should* speak in a Hearth voice via the :8555 shim, honest about what's unconfirmed. Also carries *Which characters get hands* — the per-character tool grant, which is the other feature with that name. |
...
| [`weights-and-where-they-live.md`](weights-and-where-they-live.md) | **Where the model file lives, and Hearth's rule about it** — what a root is, why the built-in pointers at other apps' folders are paths and nothing more, the landing folder, what enrolling and un-enrolling actually write, the seven commands, changing the model end to end (enroll → render → apply → load, with the service unit rendered from the config rather than kept by hand), **the launch page's Models card** — those same four verbs as buttons, with Load and Unload — and what "missing" looks like when a file moves out from under you. |
