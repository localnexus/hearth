...
>
> **If the launch page is running, there is, and you should use it.** The **COMPANION** box on the
...
>
> **In a fresh install there is no button**, because Hearth's network door and its launch page both ship
> switched off — and Hearth is what the button talks to. So on a new machine this page is not the
> exception, it is **the** way to change who's live, until you decide to turn those switches on.
>
> That makes this page worth reading in three situations: you haven't turned on Hearth's network door and its launch page
...
(`python -m hearth.pipeline.bot`, the panel on `:65000` living inside it) or the optional **Hearth** serving process
...
> **Hearth can be pinned away from it.** If `config/serve.toml` carries a `[serve.identity]` table,
> Hearth's persona and voice come from *that* fixed selection and `active.toml` supplies only the
> model leg. A pinned Hearth will not follow this edit — by design.
...
client on Hearth) is mid-turn — **wait**. Never restart out from under a live session.

> Don't probe your model server's port for this. `llama-server`'s default is `:8080`, and the connections
...
For **Hearth** itself, restart however you run it — foreground `python -m hearth.serve`, or whatever
service manager keeps it alive on your machine (a launchd agent, a systemd user unit). With the
launch page on, its **Restart Hearth** button (or `POST /admin/daemon/restart`)
...
Hearth advertises it. Two agreeing checks — what it *serves*, and what its startup line *says*:

```bash
curl -s -H "Authorization: Bearer $(cat config/serve-token)" http://127.0.0.1:65001/v1/models
```
The returned model `id` is the active character — it should be your new pick. Then Hearth's startup
...
- **`serve.toml` `enabled = false` means Hearth won't run at all.** If a restart is followed by
  *connection refused*, that's the **switch**, not a fault — Hearth is designed to load nothing and open
  no socket when disabled. (See [When it misbehaves](when-it-misbehaves.md).)
- **Restarting your model server changes nothing about who the companion is.** Persona and voice are
...
| **Hearth** (`:65001`, optional) | Snapshots `active.toml` when **it** starts — unless `[serve.identity]` pins it | Restart Hearth (step 3) |
| **Chat clients on Hearth** | Read `/v1/models` fresh; each conversation keeps the id it opened with | **No restart.** Start a new chat and the new character id shows |

> **The mental model:** the voice loop and Hearth are *two independent readers* of the same selection
