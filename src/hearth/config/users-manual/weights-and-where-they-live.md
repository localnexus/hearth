...
| **Apply** | shows that same list plus the name it would archive the old unit under, and waits. Press again and it writes the unit — and prints the two lines, which it still does not run. The model server keeps running the OLD unit until you press Load |
| **Load** / **Unload** | the two lines, as buttons. **Load** restarts the model server on the unit Apply wrote: it stops the old one, waits for it to actually leave, then starts the new one — pressing the start line again for a little while if the system is still tearing the old one down. (A bare start pressed against a running server fails silently, which is why Load holds the two lines together.) **Unload** stops it and nothing serves until Load. Both run through the same watched-command machinery the Externals card uses, so the output goes to a log rather than to your screen. While a companion is up the card says so in words, and either button asks before bouncing the server under it |
