# GLOSSARY — terms, acronyms & shorthand

**What this is.** The plain-language decoder for this project's acronyms and shorthand — so someone
**new to this work** can read the other docs without prior context.

## How it's organized

Terms are distributed **alphabetically** across three shard files; numeric conventions get their own file:

| File | Holds |
|---|---|
| [`0-9.md`](0-9.md) | **Numeric conventions** cited across the docs. |
| [`A-M.md`](A-M.md) | Terms, acronyms & shorthand beginning **A through M**. |
| [`N-Z.md`](N-Z.md) | Terms, acronyms & shorthand beginning **N through Z** (incl. symbol-prefixed like `--new`). |

Each entry is a one-liner: **term** — expansion (for acronyms) — what it means in one line.

## Index (quick jump)

- **A–M** — A2DP · active.toml · airgap · barge-in · Chatterbox-Turbo · character · cold vs warm ·
  conditionals · config_loader · config_reload · context-budget gauge / zones · CoT · ephemeral session ·
  fp16 · FREE tier · held session · held-in-ctx · HFP · HIDEABLE tier · hold-request marker ·
  in-process · KV cache · live-config · LLM · LM Studio · logs/ · loopback-only ·
  mlx-audio · mlx_lm.server · MLX-Whisper · MoE · MTP
- **N–Z** — `--new` guard · NLTK · orphan(ed) session · overrides.toml · paralinguistic / voiced-breath
  cues · paralinguistic tag repair / normalization · paralinguistic strip log · persona · phantom-phrase ·
  PTT · reliable_context · resume · RSS · RTF ·
  S3Gen · session snapshot · Silero · Smart Turn · STT · TCC · "the box" · TPS · TTFA · TTFT · TTS ·
  VAD · voice descriptor / voice bundle / voice.toml · WER · WSOLA

## See also

- [bring-your-own-voice.md](../bring-your-own-voice.md) — add-a-voice recipe (origin for
  `character` / `voice bundle` / `voice.toml`).
