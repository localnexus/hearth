...
| **`config/models/<model>/model.toml`** | **the model's load facts**: `id`, `temperature`, `reasoning_effort`, `needs_template_edit`, `no_kv_reuse`, `reliable_context` (the panel's gauge line — see [LLM](llm.md)) (`context_length` is deliberately NOT here — the live LM Studio value wins) |
...
| `bot.py` **Configuration block** (`~L104–L166`) | LLM endpoint/token — module-level constants (model id now `_CFG.model_id` from config) |
...
| **env** `LM_API_TOKEN` | LM Studio token (not in the file) |
...
| Change the **LLM model** | `model =` | `config/active.toml` → `config/models/<model>/model.toml` `id` | loaded in LM Studio; **no emitted CoT** — instruct, or hybrid w/ thinking off ([LLM](llm.md)); **restart** |
| Force **hybrid thinking OFF** | `reasoning_effort = "none"` | `config/models/<model>/model.toml` | needed for hybrid-thinking models; some uncensored re-quants ignore it → LM Studio Prompt-Template edit instead ([LLM](llm.md)) |
...
| Set the **panel's context-budget line** | `reliable_context` | `config/models/<model>/model.toml` | measured usable ceiling the `Tokens` gauge counts against (`128000` today, PROVISIONAL); absent → panel falls back to the advertised window. See [LLM](llm.md) |
...
| Point at a **different LLM backend** | `LMSTUDIO_BASE_URL` / `LM_API_TOKEN` | bot · L104 / L108 | — |
...
2. **No emitted chain-of-thought** — pure instruct, or a hybrid with thinking forced off (`reasoning_effort:"none"`; uncensored re-quants need the template edit). A model that streams `reasoning_content`/`<think>` with empty `content` silently stalls the loop.
...
| [llm.md](llm.md) | §B — The LLM: model selection, the no-chain-of-thought rules, hybrid thinking off, persona, creativity, backend/token. |
