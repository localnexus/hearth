...
- **The commands:** `python -m hearth.weights roots | scan [--root NAME] [--ctx N] [--json] | list | enroll <model> (--path P | --key KEY) [--mmproj auto|none|PATH] [--yes] | unenroll <model> [--yes] | check [<model>]`. Writes preview first and need `--yes`. A write that removes or replaces a reference (`unenroll`, or `enroll` over a file already carrying one) first copies the whole `model.toml` beside itself as `model.toml.prev-<date>` — the unit's own archive rule — and both the CLI and the card name the copy before and after; nothing deletes an archive.
...
The card's **Load** and **Unload** buttons are the two lines as built-in actuators (`supervisor/models/door.py`): `door-unload` is the `bootout`, `door-load` the `bootstrap`, and `door-reload` — what Load presses — the two held together as a *stepped* actuator: bootout (exit 3, "not loaded", counts as done), wait until nothing answers on the door's port, then `bootstrap`, pressed again once a second for a bounded while if launchd is still tearing the old job down. `bootout` returns the moment launchd accepts the order, seconds before a large door has actually left, so a bare `bootstrap` pressed right after fails with "already loaded" — the reload exists for that gap. All three carry `guard = "companion"`; the runner presses the launchctl argv, and no module ever calls it.
