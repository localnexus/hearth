...
Hearth already sends **`reasoning_effort`** from the active `model.toml` on **every request** (`config_loader` → the LLM `Settings` `extra`). Set `reasoning_effort = "none"` there and that is normally the whole job.
