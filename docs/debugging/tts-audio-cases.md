...
**Symptom:** you speak over a long bot reply; the audio stops, but the *next* turn seems to wait a beat.

**Not a bug — a known limit.** `run_tts`'s `synth_future.cancel()` is best-effort: `concurrent.futures.Future.cancel()` cannot interrupt an **already-running** job, so the in-flight `generate()` finishes on the single executor thread before the next synth starts. pipecat still cancels the LLM stream and drops the queued `TTSAudioRawFrame`s, so the *audio* stops promptly — but the executor is busy for the tail of the current sentence (≤ ~1.3 s at RTF 0.24). The tighter fix is a **cooperative stop-flag** checked inside the generate loop. To confirm this is what you're seeing (not a wedge): Play 6 shows the interruption fired and the LLM cancelled; Play 1 shows the `mlx-tts` thread finishing one `generate()` then going idle.
...
**Symptom:** launch succeeds, `PipelineWorker ... ready`, but `LocalAudioInputTransport` throws `[Errno -9996] Invalid input device (no default output device)` and the bot can't hear or speak.
