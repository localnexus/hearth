...
2. `ls sessions/` — does `work-chat.json` or `session-work-chat.json` exist?
...
**Confirm:** `python3 -m json.tool sessions/<file>.json` — fails = corrupt.

**Fix:** `rm sessions/<file>.json` and start fresh. Fallback to fresh is by design; nothing crashes.

### Fault S5 — where the transcript lives; how to purge it

**Transcript path:** `sessions/<id>.json` — plaintext message list, `0600`, gitignored. The persona prompt is NOT stored.

**Purge options:**
- `./stop.sh` — deletes the ephemeral session on graceful stop (default).
- `./stop.sh --discard-held [name|--all]` — true-delete a held session.
- Manual: `rm sessions/<file>.json` for an orphan you've confirmed you don't need.

`sessions/` is `0700`, gitignored, never exposed over the `:65000` web endpoint. If it ever gets accidentally un-ignored, re-add `sessions/` to `.gitignore` immediately.
