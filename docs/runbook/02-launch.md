...
```
(The companion talks to `llama-server` at `http://127.0.0.1:8080/v1` by default — `LM_BASE_URL` / `LM_API_TOKEN` / `LM_PROVIDER` override it, see `start.sh`. Run it in an **iTerm** window — the app that holds the mic grant. Prepend `T4_METRICS=1` to print per-turn STT latency to stderr.)
...
> re-routes a "default" launch to a different server or data folder with no warning; the only visible
...
**Confirm it's live:** speak a sentence, pause. Within **~2–3 s** you hear a reply (there is **no auto-greeting** — the companion waits for you to speak first). A good turn in the log reads:
```
...
> **First-ever synth on a fresh machine** adds a one-time **~15.8 s** MLX Metal-kernel JIT compile (cache persists on disk afterward). On this machine it's already paid — expect fast first turns unless your model server has to load the model (turn 1 only).
