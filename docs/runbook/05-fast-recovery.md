...
| `model_not_found` in the server log (LM Studio) | wrong model id | fix `id` in the active `config/models/<model>/model.toml` to the id the server reports **verbatim** (live model dir chosen by `model =` in `config/active.toml`), then restart |
| `401` / auth error to the LLM server | the server wants a key you didn't pass (or it expired) | launch with `LM_API_TOKEN=<key> ./start.sh` (llama-server `--api-key`; LM Studio's token), relaunch |
| Connection refused at `:8080` | no server running there | start `llama-server`, or set `LM_BASE_URL` to where your server listens |
| Two mic-using apps fight / no input | a stale bot still running | `./stop.sh` (or `pkill -f "python[0-9.]* -m hearth\.pipeline\.bot"`), then relaunch |
