...
The loop is a single foreground python process. Stopping it **releases the microphone** and frees the **in-process STT + TTS weights** (both live inside `bot.py` now); your model server keeps running.
