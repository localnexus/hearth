...
it — the on/off files absent means nothing loads and the composed prompt is
byte-identical.
...
and vice versa). They are files readable only by you (`0600`) in a `0700`
directory under your data folder, gitignored, local-only — the same sensitivity
