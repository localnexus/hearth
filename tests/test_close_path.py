...
class _Seam:
    ...
    def on_session_end(self, messages, store=None, *, outcome=None):
        ...
        if outcome is not None:
            outcome.update({"result": "kept", "index": "ok", "intent": "ok"})
...
class TestClosePath(unittest.TestCase):
    ...
    def test_memory_failure_still_closes_the_seam_after_finalize_ran(self):
        ...
        with self.assertRaises(RuntimeError):
            close_path.run_close(_Store(), seam, [],
                                 finalize=lambda st, m, outcome=None: log.append("finalize") or "ok",
